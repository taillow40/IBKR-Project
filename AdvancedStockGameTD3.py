...
class AdvancedStockGameTD3(gym.Env):
    def __init__(self, data_file_path):
        super(AdvancedStockGameTD3, self).__init__()
        ...
        self.action_space = gym.spaces.Box(low=0, high=1, shape=(self.stocks_number,))
    ...
    def step(self, action):
        ...
        for i in range(len(action)):
            if(action[i] == 1):
                ...
                elif(self.current_observation['portfolio'][i] > 0):
                    self.sell(i)
                else:
                    self.buy(i)
            else:
        ...
        return self.current_observation, reward, done, False, {'data': current_data, 'time': self.stock_timestamps[self.start_index + self.current_index]}
    ...
    def roll_into_observation(self):
        ...
        return self.current_index >= len(self.data_block[0]) or self.current_index >= self.episode_length
